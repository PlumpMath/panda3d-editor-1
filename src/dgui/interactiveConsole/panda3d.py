...
class panda3dIOClass( DirectObject.DirectObject ):
  ...
  # event prefix (when the console is enabled all events will have the prefix in front)
  # thus disabling the active event handlers
  ...
  def __init__( self, parent ):
    ...
    # will fail is panda3d has not been started
    ...
    try:
      ...
      print "W: interactiveConsole.__init__: window-event test failed"
...
- BUGS       : if you paste a to long text, the entry blocks
  ...
  def toggleConsole( self ):
    ...
    if hidden:
      ...
      self.accept( self.event_prefix+'page_down-repeat', self.scroll, [5] )
      # the window event has to work while the console is active and inactive
      self.accept( self.event_prefix+'window-event', self.windowEvent)
      self.accept( 'window-event', self.windowEvent)
      
      self.accept( self.event_prefix+'arrow_up'  , self.scrollCmd, [ 1] )
      self.accept( self.event_prefix+'arrow_down', self.scrollCmd, [-1] )
      
      self.accept( self.event_prefix+self.gui_key, self.toggleConsole )
      self.accept( self.event_prefix+self.autocomplete_key, self.autocomplete )
      self.accept( self.event_prefix+self.autohelp_key, self.autohelp )
      
      # accept v, c and x, where c & x copy's the whole console text
      #messenger.toggleVerbose()
      #for osx use ('meta')
      if sys.platform == 'darwin':
        self.accept( self.event_prefix+'meta', self.unfocus )
        self.accept( self.event_prefix+'meta-up', self.focus )
        self.accept( self.event_prefix+'meta-c', self.copy )
        self.accept( self.event_prefix+'meta-x', self.cut )
        self.accept( self.event_prefix+'meta-v', self.paste )
      #for windows use ('control')
      if sys.platform == 'win32' or sys.platform == 'linux2':
        self.accept( self.event_prefix+'control', self.unfocus )
        self.accept( self.event_prefix+'control-up', self.focus )
        self.accept( self.event_prefix+'control-c', self.copy )
        self.accept( self.event_prefix+'control-x', self.cut )
        self.accept( self.event_prefix+'control-v', self.paste )
