...
class SceneGraphBrowser(DirectObject):
  ...
  def __init__(self, parent=None, root=None, command=None, contextMenu=None,
    ...
    self.childrenFrame = DirectScrolledFrame(
                 ...
                 manageScrollBars=0, enableEdit=0, suppressMouse=DEFAULT_SUPRESS_MOUSE_OVER_GUI, sortOrder=1000 )
    ...
    SliderBG=DirectFrame( parent=self.childrenFrame,frameSize=(-.025,.025,-self.frameHeight,0),
                 frameColor=(0,0,0,.7), pos=(-.03,0,0),enableEdit=0, suppressMouse=DEFAULT_SUPRESS_MOUSE_OVER_GUI)
    # slider thumb track
    ...
    DirectButton(parent=self.rootTitle,
               ...
               enableEdit=0, suppressMouse=DEFAULT_SUPRESS_MOUSE_OVER_GUI,command=self.collapseAllTree)
    # expand-all button
    DirectButton(parent=self.rootTitle,
               frameColor=(0,.8,1,1),frameSize=(-.4,.4,-.4,.4),
               pos=(-.5*self.frameWidth+.075,0,.03), scale=.05,
               text='+', text_pos=(-.05,-.22), text_scale=(1.2,1.2), text_fg=(0,0,0,1),
               enableEdit=0, suppressMouse=DEFAULT_SUPRESS_MOUSE_OVER_GUI,command=self.expandAllTree)
...
#       self.accept(DGG.WITHOUT+self.childrenFrame.guiId,self.__exitingFrame)
  ...
  def __listchildren(self,np,level,editable=True):
    ...
    try:
      ...
      print "W: SceneGraphBrowser.__listchildren: bug with 1.5.4???"
      print "  - on nodepath:", np
      traceback.print_exc()
    if status!=0:
      if np!=self.root:
    ...
    for c in self.childrenList:
        ...
        if c.status==-1:
           c.treeButton = DirectButton(parent=c.nodeButton,
               ...
               enableEdit=0, suppressMouse=DEFAULT_SUPRESS_MOUSE_OVER_GUI,command=self.__collapseTreeNupdate,extraArgs=[idx])
        idx+=1
